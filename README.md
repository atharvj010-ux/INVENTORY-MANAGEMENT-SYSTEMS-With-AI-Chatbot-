...
```mermaid
flowchart TD
    A[Visitor opens app] --> B[Landing page]
    B --> C[Sign up or Login]
    C --> D[Firebase Auth success]
    D --> E[auth cookie set]
    E --> F[Dashboard routes unlocked]
    F --> G[Inventory CRUD in Firestore]
    F --> H[Analytics and AI Insights]
    F --> I[AI Assistant Chat]
    I --> J[Chat session load or create]
    J --> K[Inventory snapshot + Chroma retrieval]
    K --> L[Prompt builder]
    L --> M[Gemini streaming response]
    M --> N[Response rendered + session saved]
```
...
```mermaid
flowchart LR
    U[AuthForm submit] --> A[signIn or signUp in firebase/auth.ts]
    A --> B[Firebase returns user]
    B --> C[auth=1 cookie set]
    C --> D[useAuth onAuthStateChanged]
    D --> E[AuthProvider context update]
    E --> F[dashboard/layout.tsx access check]
    F --> G[middleware.ts route allow]
```
...
```mermaid
flowchart LR
    U[Open Inventory page] --> A[useInventory(userId)]
    A --> B[subscribeInventory]
    B --> C[Firestore onSnapshot users/uid/inventory]
    C --> D[InventoryTable realtime update]
    D --> E[Add/Edit/Delete action]
    E --> F[addInventoryItem/updateInventoryItem/deleteInventoryItem]
    F --> C
```

### 4.4 AI chat workflow

```mermaid
flowchart LR
    U[User message] --> A[useInventoryChat.sendMessage]
    A --> B[POST /api/chat]
    B --> C[getSession]
    C --> D[prepareInventoryChat]
    D --> D1[syncInventoryToChroma]
    D --> D2[searchSimilarChunks]
    D --> D3[searchWebFallback optional]
    D --> E[buildInventoryChatPrompt]
    E --> F[generateContentStreamWithFallback]
    F --> G[SSE text chunks]
    G --> H[UI stream render]
    H --> I[saveChatMessages]
```

### 4.5 Document upload workflow

```mermaid
flowchart LR
    U[ChatUploadButton file select] --> A[useInventoryChat.uploadDocument]
    A --> B[POST /api/upload multipart]
    B --> C[validateFile]
    C --> D[parseDocument]
    D --> E[chunkText]
    E --> F[storeDocumentChunks in Chroma]
    F --> G[Upload success notice]
    G --> H[Next chat queries can retrieve uploaded chunks]
```
